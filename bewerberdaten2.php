...
?>	
	...
	<select style='width:100%; font-size: 16;'name='schulart' required='required' onchange="togglezuzugField()">
	<?php
...
?>	
	...
	<div class="flex-container">
		<div class="flex-item-drei">
			...
			<select style='width:100%; font-size: 16;' id="jahrgang" name="jahrgang" required='required'>
...
?>	
	...
	<?php
		...
		echo "<form id='form_z' method='post' action='./bewerberdaten2.php'>";
		} else {
...
function togglebetriebField() {
    var betriebValue = document.getElementById('jahrgang').value;
...
document.getElementById('schulart').addEventListener('change', function() {
    var schulart_kuerzel = this.value;
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            document.getElementById('jahrgang').innerHTML = this.responseText;
            setupBetriebChangeListener(); // Listener neu setzen
            togglebetriebField(); // Direkt ausführen, falls 'sonstiger' bereits ausgewählt ist
        }
    };
    xhttp.open("GET", "getStufen.php?schulart_kuerzel=" + schulart_kuerzel, true);
    xhttp.send();
});
...
<script>
    function togglezuzugField() {
		...
		const betriebField = document.getElementById("schulart");
