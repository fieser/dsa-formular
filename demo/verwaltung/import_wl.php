...
if (file_exists("/var/www/html/verwaltung/daten/svp_betrieb.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `betriebe`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $name2 = $teilung[3];
					...
					if ($db->exec("INSERT INTO `betriebe`
								   SET
									`id_edoo` = '$id_edoo',
									`kuerzel` = '$kuerzel',
									`name1` = '$name1',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_wl_ausbildungsberuf.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `berufe`")) {
...
if (file_exists("/var/www/html/verwaltung/daten/svp_berufe_ist.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `berufe_angebot`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $langform = $teilung[2];
						...
						if ($db->exec("INSERT INTO `berufe_angebot`
									   SET
									   `langform` = '$langform',
										`schluessel` = '$schluessel'")) {
...
if (file_exists("/var/www/html/verwaltung/daten/svp_staaten.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `staaten`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $langform = $teilung[3];
					...
					if ($db->exec("INSERT INTO `staaten`
								   SET
									`schluessel` = '$schluessel',
									`kurzform` = '$kurzform',
									`anzeigeform` = '$anzeigeform',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_sprachen.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `sprachen`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $langform = $teilung[3];
					...
					if ($db->exec("INSERT INTO `sprachen`
								   SET
									`schluessel` = '$schluessel',
									`kurzform` = '$kurzform',
									`anzeigeform` = '$anzeigeform',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_wl_vorbildung.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `vorbildung`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $langform = $teilung[4];
					...
					if ($db->exec("INSERT INTO `vorbildung`
								   SET
									`schluessel` = '$schluessel',
									`kurzform` = '$kurzform',
									`anzeigeform` = '$anzeigeform',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_schularten.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `schularten`")) {
			...
			while (!feof($file_handle)) {
			  ...
			  $langform = $teilung[4];
				...
				if ($kurzform != "kurzform") {
					...
					if ($db->exec("INSERT INTO `schularten`
								   SET
									`schluessel` = '$schluessel',
									`kurzform` = '$kurzform',
									`anzeigeform` = '$anzeigeform',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_sorge.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `sorge`")) {
...
<p>
<form method="post" action="/var/www/html/verwaltung/index.php">
