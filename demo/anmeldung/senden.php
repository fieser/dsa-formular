...
$select_an = $db->query("SELECT DISTINCT dsa_bewerberdaten.*, dsa_bildungsgang.* FROM dsa_bewerberdaten LEFT JOIN dsa_bildungsgang ON dsa_bildungsgang.id_dsa_bewerberdaten = dsa_bewerberdaten.id WHERE nachname LIKE '%$nachname%' AND vorname LIKE '%$vorname%' AND geburtsdatum = '$geburtsdatum' GROUP BY dsa_bewerberdaten.id");
	...
	*/
	...
	if ($db->exec("INSERT INTO `dsa_bewerberdaten`
					...
					if ($db->exec("INSERT INTO `dsa_bildungsgang`
						...
						echo "<div style='padding: 10px; background-color: #E0E0E0; margin-bottom: 20px;'>";
						...
						$select_tex = $db->query("SELECT * FROM senden_texte WHERE schulform = '$schulform' ORDER BY bezeichnung ASC");
						...
						foreach($select_tex as $tex) {
							...
							if ($tex['text'] != "") {
								...
								echo "</form>";
								echo "<p>&nbsp;</p>";
								echo "<form id='form_w' method='post' action='https://anmeldung.bbs1-mainz.de/demo/verwaltung/login_ad.php?test=BbsAj-55122rlp'>";
...
$select_sum = $db->query("SELECT md5, time FROM summen WHERE md5 = '$summe'");	
...
if ($treffer_an > 0) {
	...
	if ($treffer_an > 0) {
		...
		foreach ($select_an as $an) {		
			...
			$w_nr = ($w_nr + 1);
				...
				echo "<td width='20px'>";
				echo "<td>";
				echo "<select style='width:100%; font-size: 16;' name='prio_".$w_nr."'>";
				//echo "<select style='width:100%; font-size: 16;' name='prio_2'>";
				//value:

				echo "<option value=''></option>";
	
			$select_an2 = $db->query("SELECT DISTINCT dsa_bewerberdaten.*, dsa_bildungsgang.* FROM dsa_bewerberdaten LEFT JOIN dsa_bildungsgang ON dsa_bildungsgang.id_dsa_bewerberdaten = dsa_bewerberdaten.id WHERE nachname LIKE '%$nachname%' AND vorname LIKE '%$vorname%' AND geburtsdatum = '$geburtsdatum' GROUP BY dsa_bewerberdaten.id");
			...
			$sf_list = $an2['schulform'];
				$select_sf = $db->query("SELECT DISTINCT name, kuerzel FROM schulformen WHERE kuerzel = '$sf_list'");
			...
			$sf_list = $an2['schulform'];
				$select_sf = $db->query("SELECT DISTINCT name, kuerzel FROM schulformen WHERE kuerzel = '$schulform'");
				foreach($select_sf as $sf) {
			...
			$select_an3 = $db->query("SELECT DISTINCT dsa_bewerberdaten.*, dsa_bildungsgang.* FROM dsa_bewerberdaten LEFT JOIN dsa_bildungsgang ON dsa_bildungsgang.id_dsa_bewerberdaten = dsa_bewerberdaten.id WHERE nachname LIKE '%$nachname%' AND vorname LIKE '%$vorname%' AND geburtsdatum = '$geburtsdatum' GROUP BY dsa_bewerberdaten.id");
	...
	<label for="dsgvo">Ich stimme der Datenerfassung gemäß der <a href='./datenschutzerklaerung.pdf' target='_blank'>Datenschutzerkärung der BBS1-Mainz</a> zu.</label>
	</fieldset>
	<p></p>
   ...
   <?php
		...
		if ($treffer_sum == 0) {
		 ...
		 echo "<p>&nbsp;</p>";
			...
			echo "<p>".$vorname." ".$nachname." hat sich bereit für diese Schulform angemeldet.<br>
