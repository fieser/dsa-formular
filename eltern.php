...
if (isset($_POST['sorge1_nachname'])) {
	...
	//PLZ und Wohnort prüfen (Sorge1):
	$plz1 = $_SESSION['sorge1_plz'];

    
	
	$select_pwo = $db_temp->query("SELECT * FROM plz_ort WHERE plz = '$plz1'");
	...
	if ($treffer_pwo > 0) {
		...
		foreach($select_pwo as $pwo) {
			...
			$_SESSION['sorge1_wohnort'] = $pwo['ort'];
	...
	//PLZ und Wohnort prüfen (Sorge2):
	$plz2 = $_SESSION['sorge2_plz'];
	
	
	$select_pwo2 = $db_temp->query("SELECT * FROM plz_ort WHERE plz = '$plz2'");
	$treffer_pwo2 = $select_pwo2->rowCount();
	
	if ($treffer_pwo2 > 0) {
		
		//Ortsnahmen mit WL-Eintrag aus edoo.sys überschreiben:
		foreach($select_pwo2 as $pwo2) {
			
			$_SESSION['sorge2_wohnort'] = $pwo2['ort'];
...
<?php
	...
	if ($_SESSION['volljaehrig'] == 0) {
		echo "<h1><b>Kontaktdaten der Sorgeberechtigten</b> <font style='font-size: 0.7em;'> - <i>Anmeldung ".$sf_name."</i></font></h1>";
		$required = "required";
		...
		echo "<h1><b>Kontaktdaten zusätzlicher Ansprechpersonen</b> <font style='font-size: 0.7em;'> - <i>Anmeldung ".$sf_name."</i></font></h1>";
...
<div class="flex-container">
	...
	<?php
		...
		$select_st_k = $db_temp->query("SELECT kurzform, anzeigeform FROM sorge WHERE kurzform LIKE '$kuerzel_staatsangehoerigkeit'");
		...
		echo "<option value='".$st_k['kurzform']."'>".$st_k['anzeigeform']."</option>";
			...
			$select_st = $db_temp->query("SELECT kurzform, anzeigeform FROM sorge ORDER BY anzeigeform ASC");
...
<div class="flex-container">
	...
	<div class="flex-item-left">
        ...
        <input style='width:100%;' type="text" id="sorge1_vorname" name="sorge1_vorname" value='<?php echo $_SESSION["sorge1_vorname"] ?>' <?php echo $required ?> pattern="[A-ZÖÄÜ].*"><br>
	</div>
		
	<div class="flex-item-right">
			<label for="sorge1_nachname">Nachname:</label><br>
			<input style='width:100%;' type="text" id="sorge1_nachname" name="sorge1_nachname" value='<?php echo $_SESSION["sorge1_nachname"] ?>' <?php echo $required ?> pattern="[A-ZvedÖÄÜ].*"><br>
	</div>
</div>
	...
	<input style='width:100%;' type="text" id="sorge1_strasse" name="sorge1_strasse" value='<?php echo $_SESSION["sorge1_strasse"] ?>' <?php echo $required ?>><br>
		<?php
...
<div class="flex-container">
	...
	<div class="flex-item-left">
        ...
        <input style='width:100%;' type="text" id="sorge2_vorname" name="sorge2_vorname" value='<?php echo $_SESSION["sorge2_vorname"] ?>' pattern="[A-ZÖÄÜ].*" ><br>
	</div>
		
	<div class="flex-item-right">
			<label for="sorge2_nachname">Nachname:</label><br>
			<input style='width:100%;' type="text" id="sorge2_nachname" name="sorge2_nachname" value='<?php echo $_SESSION["sorge2_nachname"] ?>' pattern="[A-ZvedÖÄÜ].*" ><br>
	</div>
</div>
