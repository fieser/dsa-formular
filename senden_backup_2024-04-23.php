	...
	*/
	...
	if (isset($_POST['upload_submit'])) {
		...
		if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES['uploaded_file'])) {
			...
			$destFile = $uploadDir.$summe_o_sf.'.pdf.enc';
			$fileName = $summe_o_sf.'.pdf.enc';
			...
			if (encryptFile($uploadedFile, $newFilePath, $password)) {
				echo "<p><b>Datei erfolgreich hochgeladen und verschlüsselt.</b></p>";
				if ($fileCounter == 1) {
				...
				if ($zip->open($zipFile, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
					exit("Konnte ZIP-Datei nicht öffnen oder erstellen.");
				}
			...
			if (isset($_POST['upload_submit'])) {
				echo "Keine Datei zum Hochladen erhalten.";
			}
		}
								//Buttons:
