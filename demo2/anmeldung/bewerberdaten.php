...
$select_sf = $db->query("SELECT name FROM schulformen WHERE kuerzel = '$sf_kuerzel'");
    ...
    echo "<h1><b>Bewerberdaten</b> <font style='font-size: 0.7em;'> - <i>Anmeldung zur ".$sf_name."</i></font></h1>";
...
<div class="flex-container">
	...
	<?php
		...
		$select_st_k = $db->query("SELECT anzeigeform, langform FROM geschlecht WHERE anzeigeform LIKE '$kuerzel_staatsangehoerigkeit'");
		...
		echo "<option value='".$st_k['anzeigeform']."'>".$st_k['langform']."</option>";
			...
			$select_st = $db->query("SELECT anzeigeform, langform FROM geschlecht ORDER BY langform ASC");
...
<div class="flex-container">
	...
	<?php
		...
		$select_st_k = $db->query("SELECT anzeigeform, langform FROM staaten WHERE anzeigeform LIKE '$kuerzel_geburtsland'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['anzeigeform']."'>".$st_k['langform']."</option>";
		}
		} else {
			...
			$select_st = $db->query("SELECT anzeigeform, langform FROM staaten ORDER BY langform ASC");
			$treffer_st = $select_st->rowCount();
...
?>	
	...
	<?php
		...
		$select_st_k = $db->query("SELECT anzeigeform, langform FROM staaten WHERE anzeigeform LIKE '$kuerzel_staatsangehoerigkeit'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['anzeigeform']."'>".$st_k['langform']."</option>";
		}
		} else {
...
?>	
 ...
 <div class="flex-container">
	...
	<?php
		...
		$select_st_k = $db->query("SELECT anzeigeform, langform FROM sprachen WHERE anzeigeform LIKE '$kuerzel_muttersprache'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['anzeigeform']."'>".$st_k['langform']."</option>";
		}
		} else {
			...
			$select_st = $db->query("SELECT anzeigeform, langform FROM sprachen ORDER BY langform ASC");
			$treffer_st = $select_st->rowCount();
...
?>	
	...
	<?php
		...
		$select_st_k = $db->query("SELECT anzeigeform, langform FROM staaten WHERE anzeigeform LIKE '$kuerzel_herkuftsland'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['anzeigeform']."'>".$st_k['langform']."</option>";
		}
		} else {
...
?>	
	...
	<?php
		...
		$select_st_k = $db->query("SELECT kurzform, anzeigeform FROM religion WHERE kurzform LIKE '$kuerzel_staatsangehoerigkeit'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['kurzform']."'>".$st_k['anzeigeform']."</option>";
			...
			$select_st = $db->query("SELECT kurzform, anzeigeform FROM religion ORDER BY sortierung ASC");
			$treffer_st = $select_st->rowCount();
...
?>	
	...
	<div class="flex-item-left">
        ...
        <input style='width:100%;' type="text" id="plz" name="plz" value='<?php echo $_SESSION["plz"] ?>' pattern="[0-9]+" minlength="5" maxlength="5" required><br>
	</div>
	...
	<?php
		...
		$select_st_k = $db->query("SELECT kurzform, anzeigeform, langform FROM schularten WHERE kurzform LIKE '$kuerzel_staatsangehoerigkeit'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['kurzform']."'>".$st_k['langform']."</option>";
			...
			$select_st = $db->query("SELECT kurzform, anzeigeform, langform FROM schularten ORDER BY langform ASC");
			$treffer_st = $select_st->rowCount();
...
?>	
		...
		<div class="flex-item-right">
				...
				<input style='width:100%;' type="text" id="schulname" name="schulname" value='<?php echo $_SESSION["schulname"] ?>' required><br>
		</div>
	</div>
	...
	<?php
		...
		$select_st_k = $db->query("SELECT kurzform, anzeigeform, langform FROM klassenstufen WHERE kurzform LIKE '$kuerzel_staatsangehoerigkeit'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['kurzform']."'>".$st_k['anzeigeform']."</option>";
		}
		} else {
			...
			$select_st = $db->query("SELECT kurzform, anzeigeform, langform FROM klassenstufen ORDER BY schluessel ASC");
			$treffer_st = $select_st->rowCount();
...
?>	
	...
	<?php
		...
		$select_st_k = $db->query("SELECT kurzform, anzeigeform, langform FROM vorbildung WHERE kurzform LIKE '$kuerzel_staatsangehoerigkeit'");
		foreach($select_st_k as $st_k) {
		echo "<option value='".$st_k['kurzform']."'>".$st_k['langform']."</option>";
		}
		} else {
			...
			$select_st = $db->query("SELECT kurzform, anzeigeform, langform FROM vorbildung ORDER BY langform ASC");
			$treffer_st = $select_st->rowCount();
