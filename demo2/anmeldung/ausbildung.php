...
if (isset($_POST['beruf'])) {
	...
	//Ermitteln der Berufsbezeichnung:
		$beruf = $_POST['beruf'];
		$select_ber = $db->query("SELECT anzeigeform FROM berufe WHERE schluessel = '$beruf'");	
...
<body>
    <h1>Angaben zum Ausbildungsvertrag</h1>
...
<div class="flex-container">
	...
	<?php
		...
		$select_st_k = $db->query("SELECT anzeigeform, langform FROM dauer WHERE anzeigeform LIKE '$kuerzel_dauer'");
		...
		echo "<option value='".$st_k['anzeigeform']."'>".$st_k['langform']."</option>";
			...
			$select_st = $db->query("SELECT anzeigeform, langform FROM dauer ORDER BY langform ASC");
...
<div class="flex-container">
	...
	<div class="flex-item-drei">
		...
		$select_bu_s = $db->query("SELECT schluessel, langform FROM berufe WHERE schluessel LIKE '$schluessel_beruf'");
...
<?php
					...
					$select_bes = $db->query("SELECT langform, schluessel FROM berufe_angebot ORDER BY langform ASC");
...
<div class="flex-container">
 ...
 <div class="flex-container">
        ...
        <select style='width:100%; font-size: 16;' id="betrieb" name="betrieb" required='required' onchange="togglebetriebField()">
		<?php
		if ($_SESSION['betrieb'] != "") {
			$kuerzel_betrieb = $_SESSION['betrieb'];
		$select_be_k = $db->query("SELECT kuerzel, name1, name2 FROM betriebe WHERE kuerzel LIKE '$kuerzel_betrieb'");
		...
		<option value="sonstiger">sonstiger</option>
<?php
			$select_be = $db->query("SELECT kuerzel, name1, name2 FROM betriebe ORDER BY name1 ASC");
			$treffer_be = $select_be->rowCount();
			
					
			
			foreach($select_be as $be) {
				if (substr($be['name1'],0,3) != "ZZZ" AND substr($be['name1'],0,3) != "AAA") {
				 echo "<option value='".$be['kuerzel']."'>".$be['name1']."</option>";
				}
			}
?>		
