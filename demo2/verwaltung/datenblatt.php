...
if (isset($_POST['bemerkungen'])) {
		...
		if ($temp_status_voll != 1) {
			...
			if ((($an['schulform'] == "bos1" OR $an['schulform'] == "bos1" OR $an['schulform'] == "fs") AND ($dok_lebenslauf + $dok_ausweis + $dok_erfahrung + $do_zeugnis) == 4) OR (($an['schulform'] != "bos1" OR $an['schulform'] != "bos2" OR $an['schulform'] != "fs") AND ($dok_lebenslauf + $dok_ausweis + $do_zeugnis) == 3)) {
			//if ($treffer_vo > 0) {
...
$select_bew = $db->query("SELECT * FROM dsa_bewerberdaten WHERE id = '$id'");	
		...
		//$select_bil_0 = $db->query("SELECT * FROM dsa_bildungsgang WHERE id_dsa_bewerberdaten = '$id'");
			$select_bil_0 = $db->query("SELECT sf.name FROM dsa_bildungsgang bg, schulformen sf WHERE bg.id_dsa_bewerberdaten = '$id' AND bg.schulform = sf.kuerzel");
...
?>
   ...
   <figure>
	  ...
	  echo "<table>";
	  echo "<tr'><td style='padding: 5;'><b>Geshlecht:</b></td>
	  ...
	  $sorge2_art = $bew['sorge2_art'];
		...
		$select_so1 = $db->query("SELECT anzeigeform FROM sorge WHERE kurzform = '$sorge1_art'");	
		foreach($select_so1 as $so1) {
		$sorge1 = $so1['anzeigeform'];
		}
		$select_so2 = $db->query("SELECT anzeigeform FROM sorge WHERE kurzform = '$sorge2_art'");	
		...
		foreach($select_bil as $bil) {
			...
			if ($bil['schulform'] == "bs") {
			  ...
			  echo "<tr'><td style='padding: 5;'><b>Beschäftigungsdauer:</b></td>
					...
					$beruf = $bil['beruf'];
					$select_ber1 = $db->query("SELECT anzeigeform FROM berufe WHERE schluessel = '$beruf'");	
					...
					//Ermitteln der Berufsbezeichnung:
					$betrieb = $bil['betrieb'];
					$select_betr = $db->query("SELECT name1 FROM betriebe WHERE kuerzel LIKE '$betrieb'");	
					foreach($select_betr as $betr) {
						$betrieb_anz = $betr['name1'];
					}
					
					
					echo "<tr><td style='padding: 5;'><b>Ausbildung von</b></td>			
...
<div class='flex-container'>
	...
	<input type='hidden' name='id' value='<?PHP echo "$id" ?>'>
		...
		<?php
			...
			<?php
			if ($bil['schulform'] == "bos1" OR $bil['schulform'] == "bos2" OR $bil['schulform'] == "dbos" OR $bil['schulform'] == "fs") {
...
if ($bew['geschlecht'] == "W") {
$bodytext = "Sehr geehrte Frau ".$bew['nachname'].",\n";
}

if ($bew['geschlecht'] == "M") {
$bodytext = "Sehr geehrter Herr ".$bew['nachname'].",\n";
}
...
if ($bil['schulform'] != "bs" AND ($dok_ausweis != 1 OR $dok_lebenslauf != 1 OR $dok_zeugnis != 1 OR ($dok_erfahrung != 1 AND ($bil['schulform'] == "fs" OR $bil['schulform'] == "bos1" OR $bil['schulform'] == "bos2" OR $bil['schulform'] == "dbos")))) {
...
$bodytext .= "\n";
	...
	if ($dok_erfahrung != 1 AND ($bil['schulform'] == "fs" OR $bil['schulform'] == "bos1" OR $bil['schulform'] == "bos2" OR $bil['schulform'] == "dbos")) {
...
$bew_md5 = $bew['md5'];	
$select_md5 = $db->query("SELECT * FROM summen WHERE (md5 = '$bew_md5')");
...
<?php
   ...
   header("Location: ./login_ad.php?back=angebot_liste");
