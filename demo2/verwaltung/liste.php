...
foreach($select_an as $an) {
	...
	*/
	...
	echo "<tr bgcolor='' onmouseover=\"this.style.backgroundColor='d3d3d3'\" onclick=\"window.open('./datenblatt.php?id=".$id."','Fenster')\" onmouseout=\"this.style.backgroundColor=''\">";
	...
	if ($an['schulform'] != "bs") {
		...
		if ($treffer_edoo > 0) {
			...
			//Ohne Austritt:
			$select_edoo = $db->query("SELECT id FROM edoo_schueler WHERE geburtsdatum = '$geburtsdatum' AND nachname = '$nachname' AND nachname = '$nachname' AND austritt NOT LIKE '%2%'");	
			$treffer_edoo = $select_edoo->rowCount();
			...
			//Mit Austritt:
			$select_edoo_a = $db->query("SELECT id FROM edoo_schueler WHERE geburtsdatum = '$geburtsdatum' AND nachname = '$nachname' AND vorname = '$vorname' AND austritt LIKE '%2%'");	
