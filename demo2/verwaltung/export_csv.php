...
$select_bz = $db->query("SELECT kurzform FROM edoo_bewerbungsziel WHERE anzeigeform LIKE '$bgy_sp1' ORDER BY anzeigeform ASC");
foreach($select_bz as $bz) {
	$bgy_sp1 = $bz['kurzform'];
}
$select_bz = $db->query("SELECT kurzform FROM edoo_bewerbungsziel WHERE anzeigeform LIKE '$bgy_sp2' ORDER BY anzeigeform ASC");
foreach($select_bz as $bz) {
	$bgy_sp2 = $bz['kurzform'];
}
$select_bz = $db->query("SELECT kurzform FROM edoo_bewerbungsziel WHERE anzeigeform LIKE '$bgy_sp3' ORDER BY anzeigeform ASC");
foreach($select_bz as $bz) {
	$bgy_sp3 = $bz['kurzform'];
}
...
if ($bgy_sp1 == "") {
	if ($an['schulform'] == "bvj") {
		$select_bz = $db->query("SELECT kurzform FROM edoo_bewerbungsziel WHERE id_bildungsgang LIKE '1058_810105%' ORDER BY anzeigeform ASC");
			foreach($select_bz as $bz) {
				$bgy_sp1 = $bz['kurzform'];
			}
	...
	if ($an['schulform'] == "dbos") {
		$select_bz = $db->query("SELECT kurzform FROM edoo_bewerbungsziel WHERE id_bildungsgang LIKE '1058_8800000' ORDER BY anzeigeform ASC");
			foreach($select_bz as $bz) {
				$bgy_sp1 = $bz['kurzform'];
			}
	...
	if ($an['schulform'] == "bf2") {
		$select_bz = $db->query("SELECT kurzform FROM edoo_bewerbungsziel WHERE id_bildungsgang LIKE '1058_8207000T' ORDER BY anzeigeform ASC");
			foreach($select_bz as $bz) {
				$bgy_sp1 = $bz['kurzform'];
			}
	...
	$zeile = $nummer.";".$an['nachname'].";".$an['vorname'].";".$an['geschlecht'].";".date("d.m.Y",strtotime($an['geburtsdatum'])).";".$an['geburtsort'].";".$an['geburtsland'].";".$zuzug.";".$an['staatsangehoerigkeit'].";".$an['muttersprache'].";".$an['religion'].";".$an['herkuftsland'].";".$an['strasse'].";".$an['plz'].";".$an['wohnort'].";".$an['hausnummer'].";".$an['telefon1'].";".$an['telefon2'].";".$an['mail'].";".$an['schulart'].";".$an['jahrgang'].";".$an['abschluss'].";".$an['sorge1_vorname'].";".$an['sorge1_nachname'].";".$an['sorge1_strasse'].";".$an['sorge1_hausnummer'].";".$an['sorge1_plz'].";".$an['sorge1_wohnort'].";".$an['sorge1_telefon1'].";".$an['sorge1_telefon2'].";".$an['sorge1_mail'].";".$an['sorge2_vorname'].";".$an['sorge2_nachname'].";".$an['sorge2_strasse'].";".$an['sorge2_hausnummer'].";".$an['sorge2_plz'].";".$an['sorge2_wohnort'].";".$an['sorge2_telefon1'].";".$an['sorge2_telefon2'].";".$an['sorge2_mail'].";".$an['schulform'].";".$an['dauer'].";".$an['beginn'].";".$an['ende'].";".$an['beruf'].";".$an['betrieb'].";".$bgy_sp1.";".$bgy_sp2.";".$bgy_sp3.";".$fs1.";".$fs1_von.";".$fs1_bis.";".$fs2.";".$fs2_von.";".$fs2_bis.";".$fs3.";".$fs3_von.";".$fs3_bis."; \n";
...
echo "<p>".$treffer_an." Schülerdatensätze in CSV-Datei geschrieben:<br></p>";
...
echo "<h3 style='margin-top: 2em;'>Laden Sie sich die soeben generierte csv-Datei herunter:</h3>";

// Array durchlaufen und in einer Liste ausgeben
for($index=0; $index < $indexCount; $index++) {
$extension = substr($dirArray[$index], -3);
	if ($extension == 'csv'){ // nur csv Dateien ausgeben
	...
	echo  "<p style='margin-top: 1em;'><font color='".$farbe."'><b><a href='./export/".$dirArray[$index]."'>".$dirArray[$index]."</a></b></font> ".date ('d.m.Y H:i', filemtime("./export/".$dirArray[$index]))."</p>";
	}
	
}

echo "<h3 style='margin-top: 2em;'>Diese Dateien müssen nur einmalig heruntergeladen werden:</h3>";

// Array durchlaufen und in einer Liste ausgeben
for($index=0; $index < $indexCount; $index++) {
$extension = substr($dirArray[$index], -3);

		if ($extension == 'exe' OR $dirArray[$index] == "Bewerber_Import_leer.xls"){ // nur csv Dateien ausgeben
	...
	echo  "<p style='margin-top: 2em;'><font color='".$farbe."'><b><a href='./export/".$dirArray[$index]."'>".$dirArray[$index]."</a></b></font> ".date ('d.m.Y H:i', filemtime("./export/".$dirArray[$index]))."</p>";
	}
}
echo "<ol style='margin-top: 2em;'>";
echo "<li><p>Speichern Sie <b>alle drei Dateien im selben Verzeichnis</b> und starten Sie die Datei <i>bewerber_csv2bewerber_xls.exe</i>.</p>";
echo "<li><p>Importieren Sie die Ergebnisdatei <i>Bewerber_Import.xls</i> über die entsprechende Funktion im Menü <i>Modulbezogene Funktionen</i> is Bewerbermodul von edoo.sys.</p>";
echo "</ol>";
?>
