...
if (file_exists("/var/www/html/verwaltung/daten/svp_dsa_schueler.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `edoo_schueler`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $wohnort = $teilung[7];
			  $mail = $teilung[8];
			  $create_user = $teilung[9];
			  $create_date = $teilung[10];
			  $update_user = $teilung[11];
			  $update_date = $teilung[12];
			  $austritt = $teilung[13];
					...
					if ($db->exec("INSERT INTO `edoo_schueler`
								   SET
									`nachname` = '$nachname',
									`vorname` = '$vorname',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_dsa_bewerber.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `edoo_bewerber`")) {
		...
		$select_gbl = $db->query("SELECT anzeigeform FROM staaten WHERE schluessel = '$geburtsland'");	
		
		foreach($select_gbl as $gbl) {
			$geburtsland = $gbl['anzeigeform'];
		}			
		
		$select_sta = $db->query("SELECT anzeigeform FROM staaten WHERE schluessel = '$staatsangehoerigkeit'");	
		
		foreach($select_sta as $sta) {
			$staatsangehoerigkeit = $sta['anzeigeform'];
		}	

		$select_hk = $db->query("SELECT anzeigeform FROM staaten WHERE schluessel = '$herkunftsland'");	
		...
		foreach($select_hk as $hk) {
			$herkunftsland = $hk['anzeigeform'];
					...
					if ($db->exec("INSERT INTO `edoo_bewerber`
								   SET
									`nachname` = '$nachname',
									`vorname` = '$vorname',
									`geburtsdatum` = '$geburtsdatum',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_bewerbungsziel.csv")) {
				...
				// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `edoo_bewerbungsziel`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $id_bildungsgang = $teilung[3];
					...
					if ($db->exec("INSERT INTO `edoo_bewerbungsziel`
								   SET
									`id_edoo` = '$id_edoo',
									`kurzform` = '$kurzform',
			...
			while (!feof($file_handle)) {
			  ...
			  $adresse = $teilung[5];
				...
				if ($typ == "1113_04") { //wenn Email
					...
					if ($db->exec("UPDATE `edoo_bewerber`
									   SET
...
if (file_exists("/var/www/html/verwaltung/daten/svp_dsa_bewerber_fremdsprachen.csv")) {
		...
		// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `edoo_fremdsprachen`")) {
			...
			while (!feof($file_handle)) {
			  ...
			  $fs_bis = $teilung[6];
				...
				$line2 = str_replace("1104_5001","Klasse 5",$line2);
			  				...
			  				if ($db->exec("INSERT INTO `edoo_fremdsprachen`
								   SET
									`nachname` = '$nachname',
									`vorname` = '$vorname',
									`geburtsdatum` = '$geburtsdatum',
...
if (file_exists("/var/www/html/verwaltung/daten/svp_dsa_schueler_m_betrieb.csv")) {
		...
		// Tabelle leeren:
					...
					if ($db->exec("TRUNCATE TABLE `edoo_schueler_betrieb`")) {
		...
		$file_handle = fopen($file_an, 'r');
			...
			while (!feof($file_handle)) {
			  ...
			  $id_betrieb = $teilung[4];
			  				...
			  				if ($db->exec("INSERT INTO `edoo_schueler_betrieb`
								   SET
									`nachname` = '$nachname',
									`vorname` = '$vorname',
									`geburtsdatum` = '$geburtsdatum',
