...
$select_sf = $db_temp->query("SELECT name FROM schulformen WHERE kuerzel = '$sf_kuerzel'");
    ...
    echo "<h1><b>Besondere Angaben zur Schulform</b> <font style='font-size: 0.7em;'> - <i>Anmeldung ".$sf_name."</i></font></h1>";
	
?>
	
	
    <form id="inputForm" action="zusatz.php" method="post" accept-charset="utf-8">
...
<p style='padding: 10px; margin-bottom: 0px;'><b>Wählen Sie Ihre Fachrichtung:</b></p>
	...
	<div class="flex-container">
		...
		$select_st = $db_temp->query("SELECT anzeigeform FROM edoo_bewerbungsziel WHERE id_bildungsgang LIKE '1058_87010%' ORDER BY anzeigeform ASC");
		$treffer_st = $select_st->rowCount();
...
<p style='padding: 10px; margin-bottom: 0px;'><b>Anerkannter Fremdspracheunterricht:</b></p>
	...
	if ($_SESSION['sorge1_nachname'] == "") {
		echo "<form id='form_z' method='post' action='./bewerberdaten.php' accept-charset='utf-8'>";
	} else {
		echo "<form id='form_z' method='post' action='./eltern.php'accept-charset='utf-8'>";
	}
	?>
