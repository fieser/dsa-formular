...
// Das Captcha wurde korrekt ausgefüllt //
		...
		if ($treffer_ab == 0) {
			...
			echo "<p>Entweder haben Sie sich vertippt oder Ihre E-Mail-Adresse wurde noch nicht von unserem Sekretariat erfasst?</p>";
			echo ' <br>';
			if ($treffer_abd > 0) {
		...
		if ($treffer_ab > 1) { //Email nicht eindeutig:
		...
		if ($treffer_ab == 1) {
			...
			//PLZ prüfen:
			$betrieb_id = $ab['ausbilder_betrieb_id'];
			
			$select_plz = $db_temp->query("SELECT * FROM betriebe WHERE (id_edoo LIKE '$betrieb_id' AND betrieb_plz = '$plz_anfrage')");
			$treffer_plz = $select_plz->rowCount();

			if ($treffer_plz > 0) { //PLZ gefunden
			...
			$zufallszahl = rand(10000,10000000);
				...
				} else { //keine PLZ gefunden


					echo "<br><h2><b>Ihre Postleitzahl konnte nicht zugeordnet werden!</b></h2>";
					echo '<br>';
...
} else {  // wenn keine Anfrage übermittelt wurde
...
echo "<h1>Zugangslink anforden</h1><br>";
	...
	?>
		...
		<table>
			...
			<tr height="40"><td><label for="email">Ihre E-Mail-Adresse:&nbsp;&nbsp;</label></td><td><input id="email" type="email" name="email" require /></td></tr>
			<tr height="20"><td></td></tr>
			<tr height="40"><td>Ihre Postleitzahl:&nbsp;&nbsp;</></td><td><input id="plz" type="text" name="plz" pattern="[0-9]+" minlength="4" maxlength="5" required /></td></tr>
			<tr height="20"><td></td></tr>
